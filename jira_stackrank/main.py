...
def main() -> int:
    ...
    try:
        ...
        if handle_noop_run(ranked, moves, args.apply, started_at):
            return 0
        ...
        if not apply_moves(client, moves, args.confirm_each):
            print_execution_summary(
                len(ranked),
                len(moves),
                apply_mode=True,
...
def apply_moves(client: RankMover, moves: list[MovePlan], confirm_each: bool) -> bool:
    print_apply_section(len(moves))
    for index, move in enumerate(moves, start=1):
        if confirm_each and not confirm_move(move):
